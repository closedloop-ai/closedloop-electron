...
import { addRepo, updateSettings as updateReposSettings, loadReposConfig } from "../server/operations/repos-config-utils.js";
...
export async function seedReposConfig(rawSandboxBaseDirectory: string): Promise<void> {
  try {
    ...
    if (!existingDir || !isWithinSandbox) {
      await updateReposSettings(
        { worktreeParentDir: sandboxBaseDirectory, worktreeParentDirConfirmed: true },
        configDir
      );
    ...
    for (const entry of entries) {
      ...
      try {
        if (statSync(dotGit).isDirectory()) {
          await addRepo(fullPath, undefined, configDir);
        }
      } catch {
