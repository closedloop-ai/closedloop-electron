...
test("replays EXECUTE completion from persisted execution-result artifacts during boot-recovery", async () => {
  ...
  // Persisted v2 envelope; fullName/pr_url must align with the schema's
  // super-refine (pr_url === https://github.com/{fullName}/pull/{pr_number}).
  await fs.writeFile(
    ...
    JSON.stringify({
      ...
      results: [
        {
          status: "success",
          fullName: "example/recovered-execute",
          pr_url: "https://github.com/example/recovered-execute/pull/123",
          pr_number: 123,
  ...
  assert.equal(
    uploadBody.artifacts?.executionResult?.results?.[0]?.branch_name,
