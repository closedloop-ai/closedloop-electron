...
async function attemptLlmCommit(
  ...
  const prompt = [
    ...
    "        - Then the Feature Flags attestation section (REQUIRED — the target repo CI workflow will reject the PR without it):",
    "          ```",
    "          ## Feature Flags",
    "",
    "          [flag:N/A] Automated ClosedLoop.AI loop — feature flags are evaluated during plan review.",
    "          ```",
...
function executeGitOperations(
  ...
  try {
    ...
    // Build PR body with Feature Flags attestation + metadata footer, written
    // to a temp file to avoid shell escaping issues (--body-file approach).
    const artifactLine =
