...
// ---------------------------------------------------------------------------
// Test 1: Context pack layout — PRD staged under context/artifacts; prompt elsewhere
// ---------------------------------------------------------------------------

test("DECOMPOSE: stages prd.md under .closedloop-ai/context/artifacts/ and prompt in decompose-prompt.txt", async () => {
  ...
  // Spy script: capture cwd, staged PRD, context prompt (should not exist), CLI prompt file
  const spyScript = [
    "#!/bin/sh",
    `echo "CWD=$(pwd)" > ${JSON.stringify(captureFile)}`,
    `echo "PRD_MD=$(cat .closedloop-ai/context/artifacts/prd.md 2>/dev/null || echo MISSING)" >> ${JSON.stringify(captureFile)}`,
    `echo "CONTEXT_PROMPT_MD=$(cat .closedloop-ai/context/prompt.md 2>/dev/null || echo MISSING)" >> ${JSON.stringify(captureFile)}`,
    `echo "DECOMPOSE_PROMPT_FILE=$(cat decompose-prompt.txt 2>/dev/null || echo MISSING)" >> ${JSON.stringify(captureFile)}`,
  ...
  const prdMd = getValue("PRD_MD=");
  assert.equal(prdMd, "The full PRD content for decomposition");

  const contextPromptMd = getValue("CONTEXT_PROMPT_MD=");
  assert.equal(contextPromptMd, "MISSING");

  const decomposePromptFile = getValue("DECOMPOSE_PROMPT_FILE=");
  assert.equal(decomposePromptFile, "Decompose this PRD into features");

  const artifactsRaw = getValue("ARTIFACTS=");
  assert.ok(artifactsRaw, "Artifacts listing should be captured");
  assert.ok(
    artifactsRaw!.includes("prd.md"),
    `Should contain staged prd.md: ${artifactsRaw}`,
  );
  assert.ok(
    !artifactsRaw!.includes("template-"),
    `TEMPLATE artifacts are not staged as separate files: ${artifactsRaw}`,
  );
});
