...
function startServer(app, port) {
  ...
  return new Promise((resolve) => {
    // CLOSEDLOOP VENDOR PATCH #1 (see vendor/agent-monitor/VENDOR.md):
    // bind loopback only. Upstream binds 0.0.0.0 (LAN-exposed); this sidecar
    // is local-only. The WebSocket server attaches to this same http.Server,
    // so this also confines /ws to 127.0.0.1.
...
if (require.main === module) {
  ...
  // CLOSEDLOOP VENDOR PATCH #2 (see vendor/agent-monitor/VENDOR.md):
  // Upstream silently writes 8 hooks into ~/.claude/settings.json on EVERY
  // startup. That is consent-bearing global config mutation. The host app
  // gates this behind explicit user opt-in and only sets CCAM_AUTO_INSTALL_HOOKS
  // when the user has enabled session tracking.
