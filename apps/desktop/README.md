...
- `pnpm -C apps/desktop dist:mac` builds the macOS artifacts on a macOS host.
- `pnpm -C apps/desktop dist:win` builds Windows NSIS installers on a Windows host, or on a non-Windows host only when the extra cross-build tooling (notably `wine`) is installed and working.
- `pnpm -C apps/desktop dist:all` just runs both targets through electron-builder; it is convenient for CI but should not be expected to work on a stock macOS laptop without the Windows cross-build toolchain.
