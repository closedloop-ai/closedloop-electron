...
//     - closedloop-runtime.js (Electron in-process runtime wrapper)
...
// extraResources so the in-process runtime and hook scripts remain real files.
...
function renderClosedLoopRuntimeSource() {
  return `"use strict";

// Generated by apps/desktop/scripts/build-agent-monitor.mjs.
// Hosts the generated Agent Monitor inside Electron main instead of spawning
// Electron-as-Node as a separate process.
...
let activeRuntimeEnvContext = null;
let activeRuntimeStart = null;

async function startClosedLoopAgentMonitorRuntime(options = {}) {
  if (activeRuntime) return activeRuntime;
  if (activeRuntimeStart) return activeRuntimeStart;

  activeRuntimeStart = startClosedLoopAgentMonitorRuntimeOnce(options).finally(() => {
    activeRuntimeStart = null;
  });
  return activeRuntimeStart;
}

async function startClosedLoopAgentMonitorRuntimeOnce(options = {}) {
  const signal = options.signal || null;
  throwIfAborted(signal);

  const rootDir = path.resolve(options.rootDir || path.join(__dirname, ".."));
  const port = parsePort(
    options.port || (options.env && options.env.DASHBOARD_PORT) || process.env.DASHBOARD_PORT || "4820",
  );
  const envState = installRuntimeContext(rootDir, {
    ...(options.env || {}),
    CCAM_RUNTIME_ROOT: rootDir,
    DASHBOARD_PORT: String(port),
    CLAUDE_DASHBOARD_PORT: String(port),
  });
  const releaseChildProcessGuard = installChildProcessEnvGuard(envState);
  clearRuntimeRequireCache(rootDir);

  let httpServer = null;
  let services = null;
  try {
    throwIfAborted(signal);
    const serverIndex = withRuntimeContext(() => require("./index"));
    if (
      typeof serverIndex.createApp !== "function" ||
      typeof serverIndex.startServer !== "function"
    ) {
      throw new Error("generated server/index.js does not export createApp/startServer");
    }

    throwIfAborted(signal);
    const app = withRuntimeContext(() => serverIndex.createApp());
    throwIfAborted(signal);
    httpServer = await withRuntimeContext(() => serverIndex.startServer(app, port));
    throwIfAborted(signal);
    services = withRuntimeContext(() => startRuntimeServices());
    throwIfAborted(signal);
...
function throwIfAborted(signal) {
  ...
  const error = new Error("Agent Monitor startup aborted");
...
function startRuntimeServices() {
  ...
  const broadcast = websocket.broadcast;
  const dbModule = require("./db");

  try {
    const { startUpdateScheduler } = require("./update-scheduler");
    handles.updateScheduler = startUpdateScheduler({ broadcast });
  } catch (err) {
    console.warn("update scheduler failed to start:", err && err.message);
  }

  handles.stopWatchers = startWatchers(broadcast);
  reconcileDashboardRuns();
  maybeInstallHooks();
  handles.maintenanceTimer = startMaintenanceSweep(dbModule, broadcast);
  runStartupBackfills(dbModule, handles);
  handles.ingestPromise = startColdStartIngest(dbModule, handles.ingestAbort.signal);

  return handles;
}

async function stopRuntimeServices(handles) {
  if (!handles) {
  ...
  if (handles.maintenanceTimer) {
    clearInterval(handles.maintenanceTimer);
    handles.maintenanceTimer = null;
  }
  if (handles.catalogFetchTimer) {
    clearInterval(handles.catalogFetchTimer);
    handles.catalogFetchTimer = null;
  }
  try {
    handles.ingestAbort.abort();
  } catch {
    /* ignore */
  }
  try {
    handles.stopWatchers && handles.stopWatchers();
  } catch {
    /* ignore */
  }
  try {
    handles.updateScheduler && handles.updateScheduler.stop();
  } catch {
    /* ignore */
  }
  stopTopLevelRuntimeSideEffects();
  ...
  await Promise.all([
    waitForPromise(handles.ingestPromise, 1000),
    waitForPromise(handles.catalogFetchPromise, 1000),
  ]);
}

function stopTopLevelRuntimeSideEffects() {
  try {
    const hooksRouter = require("./routes/hooks");
    if (typeof hooksRouter.stopWatchdog === "function") hooksRouter.stopWatchdog();
  } catch {
    /* ignore */
  }
}

function startWatchers(broadcast) {
  const specs = [
    ["cc", "./lib/cc-watcher", "startCcWatcher", "stopCcWatcher"],
    ["codex", "./lib/codex-watcher", "startCodexWatcher", "stopCodexWatcher"],
    ["cursor", "./lib/cursor-watcher", "startCursorWatcher", "stopCursorWatcher"],
    ["copilot", "./lib/copilot-watcher", "startCopilotWatcher", "stopCopilotWatcher"],
    ["opencode", "./lib/opencode-watcher", "startOpenCodeWatcher", "stopOpenCodeWatcher"],
  ];

  for (const [label, modPath, startFn] of specs) {
    try {
      const mod = require(modPath);
      if (typeof mod[startFn] === "function") mod[startFn]({ broadcast });
    } catch (err) {
      console.warn(label + "-watcher failed to start:", err && err.message);
    }
  }

  return () => {
    for (const [, modPath, , stopFn] of specs) {
      try {
        const mod = require(modPath);
        if (typeof mod[stopFn] === "function") mod[stopFn]();
      } catch {
        /* ignore */
      }
    }
...
function reconcileDashboardRuns() {
  try {
    ...
    console.warn("dashboard-runs reconciliation failed:", err && err.message);
  }
}

function maybeInstallHooks() {
  if (process.env.CCAM_AUTO_INSTALL_HOOKS !== "1") return;
  try {
...
function startMaintenanceSweep(cleanupDb, broadcast) {
  const staleMinutes = (() => {
  ...
  const sweepIntervalMs = Math.max(
    60000,
    Math.min(300000, (staleMinutes * 60000) / 4),
  );
  ...
  const timer = setInterval(() => {
    try {
      const stale = cleanupDb.stmts.findStaleSessions.all("__periodic__", staleMinutes);
      ...
      if (stale.length > 0) {
        ...
        for (const session of stale) {
          cleanupDb.stmts.updateSession.run(null, "abandoned", now, null, session.id);
          broadcast("session_updated", cleanupDb.stmts.getSession.get(session.id));
          const tpRow = cleanupDb.db
            .prepare(
              "SELECT json_extract(data, '$.transcript_path') as tp FROM events " +
                "WHERE session_id = ? AND json_extract(data, '$.transcript_path') IS NOT NULL LIMIT 1",
            )
            .get(session.id);
          if (tpRow && tpRow.tp) transcriptCache.invalidate(tpRow.tp);
        }

        for (const session of stale) {
          const agents = cleanupDb.stmts.listAgentsBySession.all(session.id);
          for (const agent of agents) {
            if (agent.status === "completed") broadcast("agent_updated", agent);
          }
        }
      }
      ...
      console.warn("[SWEEP] Maintenance sweep failed:", (err && err.message) || err);
    }
  }, sweepIntervalMs);
...
function runStartupBackfills(dbModule, handles) {
  handles.startupTimer = setTimeout(() => {
    withRuntimeContext(() => {
      handles.startupTimer = null;
      if (handles.stopped) return;

      try {
        require("./lib/plan-backfill").runClaudePlanBackfill(dbModule.db);
      } catch (err) {
        console.warn("[plans] backfill failed:", err && err.message);
      }

      try {
        require("./lib/pr-backfill").runClaudePrBackfill(dbModule.db);
      } catch (err) {
        console.warn("[pull-requests] backfill failed:", err && err.message);
      }

      try {
        require("./lib/pack-scanner").runPackScanner(dbModule.db);
      } catch (err) {
        console.warn("[packs] scanner failed:", err && err.message);
      }
      ...
      try {
        ...
        handles.catalogFetchPromise = catalogFetcher.runCatalogFetch(dbModule.db).catch(() => {});
        handles.catalogFetchTimer = catalogFetcher.scheduleCatalogFetch(dbModule.db);
      } catch (err) {
        console.warn("[catalog] startup failed:", err && err.message);
      }
    });
  }, 1000);
  if (handles.startupTimer && typeof handles.startupTimer.unref === "function") {
    handles.startupTimer.unref();
  }
}
...
function startColdStartIngest(dbModule, signal) {
  try {
    ...
    if (existingCount === 0) {
      harnesses.unshift({
        ...
        importAll: async (db, opts) => {
          const result = await importAllSessions(db, opts);
          try {
            await backfillCompactions(db);
          } catch {
            /* ignore */
          }
          return result;
    ...
    return ingestAllHarnesses({ dbModule, harnesses, signal }).catch(() => {});
  } catch (err) {
    console.warn("ingest orchestrator failed to start:", err && err.message);
    return null;
  }
}

function installRuntimeContext(rootDir, env) {
  const keys = new Set(RUNTIME_ENV_KEYS);
  ...
  for (const [key, value] of Object.entries(env || {})) {
    if (value !== undefined && value !== null) runtimeEnv[key] = String(value);
  }
  const hostEnv = process.env;
  const hostCwd = process.cwd.bind(process);
  const hostGlobalPaths = Module.globalPaths.slice();
  const hostResolveFilename = Module._resolveFilename;
  const normalizedRoot = path.resolve(rootDir);
  const runtimePrefix = normalizedRoot + path.sep;
  const state = {
    keys,
    previous,
    runtimeEnv,
    ...
    usesRuntimeContext() {
      ...
      return stack
        .split("\\n")
        .some((line) => !line.includes("closedloop-runtime.js") && line.includes(runtimePrefix));
    },
    restore() {
      process.env = hostEnv;
      process.cwd = hostCwd;
      if (activeRuntimeEnvContext === state) {
        activeRuntimeEnvContext = null;
      }
      Module._initPaths();
      Module.globalPaths.length = 0;
      Module.globalPaths.push(...hostGlobalPaths);
      Module._resolveFilename = hostResolveFilename;
    },
  };
  function runtimeNodePaths() {
    const runtimeNodePath = runtimeEnv.NODE_PATH || "";
    return String(runtimeNodePath)
      .split(path.delimiter)
      .filter((value) => value.length > 0)
      .map((value) => path.resolve(value));
  }
  function refreshRuntimeModulePaths() {
    withRuntimeContext(() => Module._initPaths());
    const runtimePaths = runtimeNodePaths();
    const next = [];
    const seen = new Set();
    for (const candidate of [...runtimePaths, ...hostGlobalPaths]) {
      if (seen.has(candidate)) continue;
      seen.add(candidate);
      next.push(candidate);
    }
    Module.globalPaths.length = 0;
    Module.globalPaths.push(...next);
  }
  Module._resolveFilename = function resolveFilename(request, parent, isMain, options) {
    try {
      return hostResolveFilename.call(this, request, parent, isMain, options);
    } catch (error) {
      const filename = parent && parent.filename ? path.resolve(parent.filename) : "";
      const fromRuntime =
        filename === normalizedRoot || filename.startsWith(runtimePrefix);
      if (!fromRuntime || !error || error.code !== "MODULE_NOT_FOUND") {
        throw error;
      }
      const fallbackPaths = [
        ...runtimeNodePaths(),
        ...((options && Array.isArray(options.paths)) ? options.paths : []),
      ];
      return hostResolveFilename.call(this, request, parent, isMain, {
        ...(options || {}),
        paths: fallbackPaths,
      });
    }
  };
  const envProxy = new Proxy(hostEnv, {
    get(target, prop, receiver) {
      if (
        typeof prop === "string" &&
        state.usesRuntimeContext() &&
        Object.prototype.hasOwnProperty.call(runtimeEnv, prop)
      ) {
    ...
    set(target, prop, value, receiver) {
      if (typeof prop === "string" && state.usesRuntimeContext()) {
        keys.add(prop);
        if (!previous.has(prop)) {
          previous.set(prop, Object.prototype.hasOwnProperty.call(target, prop)
            ? target[prop]
            : undefined);
        }
        if (value === undefined || value === null) delete runtimeEnv[prop];
        else runtimeEnv[prop] = String(value);
        refreshRuntimeModulePaths();
        return true;
      }
    ...
    ownKeys(target) {
      const own = new Set(Reflect.ownKeys(target));
      if (state.usesRuntimeContext()) {
        for (const key of Object.keys(runtimeEnv)) own.add(key);
      }
  ...
  process.cwd = function cwd() {
    return state.usesRuntimeContext() ? normalizedRoot : hostCwd();
  };
  activeRuntimeEnvContext = state;
  refreshRuntimeModulePaths();
  return state;
}

function withRuntimeContext(fn) {
  const state = activeRuntimeEnvContext;
  if (!state) return fn();
  state.depth += 1;
  try {
    return fn();
...
function installChildProcessEnvGuard(envState) {
  const previousMethods = {};

  function sanitizeOptions(options) {
    if (!envState.usesRuntimeContext()) {
      return options;
    }
    const base = options && typeof options === "object" ? options : {};
    return {
      ...base,
      env: sanitizeChildEnv(base.env || process.env, envState),
    };
  }

  for (const method of CHILD_PROCESS_METHODS) {
    if (typeof childProcess[method] !== "function") continue;
    previousMethods[method] = childProcess[method];
  }

  childProcess.spawn = function spawn(command, args, options) {
    if (Array.isArray(args)) {
      return previousMethods.spawn.call(this, command, args, sanitizeOptions(options));
    }
    return previousMethods.spawn.call(this, command, sanitizeOptions(args));
  };
  childProcess.exec = function exec(command, options, callback) {
    if (typeof options === "function") {
      return previousMethods.exec.call(this, command, sanitizeOptions(undefined), options);
    }
    return previousMethods.exec.call(this, command, sanitizeOptions(options), callback);
  };
  childProcess.execFile = function execFile(file, args, options, callback) {
    if (typeof args === "function") {
      return previousMethods.execFile.call(this, file, sanitizeOptions(undefined), args);
    }
    if (Array.isArray(args)) {
      if (typeof options === "function") {
        return previousMethods.execFile.call(this, file, args, sanitizeOptions(undefined), options);
      }
      return previousMethods.execFile.call(this, file, args, sanitizeOptions(options), callback);
    }
    return previousMethods.execFile.call(this, file, [], sanitizeOptions(args), options);
  };
  childProcess.fork = function fork(modulePath, args, options) {
    if (Array.isArray(args)) {
      return previousMethods.fork.call(this, modulePath, args, sanitizeOptions(options));
    }
    return previousMethods.fork.call(this, modulePath, [], sanitizeOptions(args));
  };
  childProcess.spawnSync = function spawnSync(command, args, options) {
    if (Array.isArray(args)) {
      return previousMethods.spawnSync.call(this, command, args, sanitizeOptions(options));
    }
    return previousMethods.spawnSync.call(this, command, sanitizeOptions(args));
  };
  childProcess.execFileSync = function execFileSync(file, args, options) {
    if (Array.isArray(args)) {
      return previousMethods.execFileSync.call(this, file, args, sanitizeOptions(options));
    }
    return previousMethods.execFileSync.call(this, file, [], sanitizeOptions(args));
  };
  childProcess.execSync = function execSync(command, options) {
    return previousMethods.execSync.call(this, command, sanitizeOptions(options));
  };
  syncChildProcessBuiltinExports();

  return () => {
    for (const [method, original] of Object.entries(previousMethods)) {
...
function syncChildProcessBuiltinExports() {
  try {
...
function sanitizeChildEnv(env, envState) {
  const next = { ...(env || {}) };
  for (const key of envState.keys) {
    if (!Object.prototype.hasOwnProperty.call(next, key)) continue;
    if (envState.previous.has(key)) {
      const previousValue = envState.previous.get(key);
...
function clearRuntimeRequireCache(rootDir) {
  const normalizedRoot = path.resolve(rootDir);
...
async function closeWebSocket() {
  try {
    const websocket = require("./websocket");
    if (typeof websocket.closeWebSocket === "function") websocket.closeWebSocket();
  } catch {
    /* ignore */
  }
}
...
async function closeHttpServer(server) {
  ...
  await new Promise((resolve) => {
    ...
    try {
      if (typeof server.__closedloopDestroyConnections === "function") {
...
function once(fn) {
  let promise = null;
  return () => {
    if (!promise) promise = Promise.resolve().then(fn);
    return promise;
  };
}
...
function parsePort(value) {
  ...
  if (!Number.isInteger(n) || n <= 0 || n > 65535) {
    throw new Error("invalid Agent Monitor port: " + value);
  }
  return n;
}

module.exports = { startClosedLoopAgentMonitorRuntime };
`;
}
...
function materializeRuntimeTree() {
  ...
  patchHooksWatchdogStop(generatedHooksRoute);
  ...
  writeFileSync(generatedRuntimeFile, renderClosedLoopRuntimeSource(), "utf8");
...
function patchServerIndex(file) {
  ...
  if (!source.includes('server.once("error", onError);')) {
    const promiseNeedle = [
      "  return new Promise((resolve) => {",
      '    server.listen(port, "127.0.0.1", () => {',
    ].join("\n");
    if (!source.includes(promiseNeedle)) {
      throw new Error(
        `Unable to patch ${file}: expected the startServer listen promise for error propagation.`,
      );
    }
    source = source.replace(
      promiseNeedle,
  ...
  if (!source.includes('server.off("error", onError);\n      initWebSocket(server);')) {
    const startNeedle = [
      "function startServer(app, port) {",
      "  const server = http.createServer(app);",
      "  initWebSocket(server);",
    ...
    if (!source.includes(startNeedle)) {
      throw new Error(
        `Unable to patch ${file}: expected startServer socket tracking block for WebSocket listen-error hardening.`,
      );
    }
    source = source.replace(
      startNeedle,
    ...
    const listenSuccessNeedle = '      server.off("error", onError);';
    if (!source.includes(listenSuccessNeedle)) {
      throw new Error(
        `Unable to patch ${file}: expected startServer success callback for WebSocket initialization.`,
      );
    }
    source = source.replace(
      listenSuccessNeedle,
      [listenSuccessNeedle, "      initWebSocket(server);"].join("\n"),
    );
  }
  ...
  if (!source.includes('require("./websocket").closeWebSocket();')) {
    ...
    if (!source.includes(shutdownNeedle)) {
      throw new Error(
        `Unable to patch ${file}: expected the shutdown cleanup block for runtime ownership hardening.`,
      );
    }
    source = source.replace(
      shutdownNeedle,
  ...
  if (!source.includes("runClaudePlanBackfill")) {
    ...
    source = source.replace(
      ...
      [
        ...
        // by it. Note: this is NOT tied to a runtime "ready" signal; it
...
function patchHooksWatchdogStop(file) {
  let source = readFileSync(file, "utf8");
  if (source.includes("router.stopWatchdog = stopWatchdog;")) return;
  ...
  if (!source.includes(timerNeedle)) {
    throw new Error(
      `Unable to patch ${file}: expected watchdog timer export block for in-process cleanup.`,
    );
  }
  source = source.replace(
    timerNeedle,
    [
      ...
      "router.transcriptCache = transcriptCache;",
      "router.watchdogCheck = watchdogCheck;",
      "router.stopWatchdog = stopWatchdog;",
      "module.exports = router;",
    ].join("\n"),
  );
...
function assertGeneratedTree() {
  ...
  if (!serverIndex.includes('server.once("error", onError);')) {
    throw new Error(
      "Generated server/index.js is missing startServer listen error propagation.",
    );
  }
  if (!serverIndex.includes('server.off("error", onError);\n      initWebSocket(server);')) {
    throw new Error(
      "Generated server/index.js must initialize WebSocket only after listen succeeds.",
    );
  }
  ...
  if (!hooksRouteSource.includes("router.stopWatchdog = stopWatchdog;")) {
    throw new Error(
      "Generated server/routes/hooks.js is missing the watchdog cleanup export.",
    );
  }

  const runtimeSource = readFileSync(generatedRuntimeFile, "utf8");
  for (const requiredRuntimeToken of [
    "startClosedLoopAgentMonitorRuntime",
    "clearRuntimeRequireCache",
    "installChildProcessEnvGuard",
    "sanitizeChildEnv",
    ...
    if (!runtimeSource.includes(requiredRuntimeToken)) {
      throw new Error(
        `Generated closedloop-runtime.js is missing ${requiredRuntimeToken}.`,
      );
    }
  }
...
if (
  ...
  existsSync(generatedRuntimeFile) &&
